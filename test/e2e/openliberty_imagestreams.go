...
func OpenLibertyImageStreamTest(t *testing.T) {
	...
	if err = openLibertyImageStreamTest(t, f, ctx); err != nil {
...
func openLibertyImageStreamTest(t *testing.T, f *framework.Framework, ctx *framework.TestCtx) error {
	const name = "open-liberty-app"
	...
	ol := util.MakeBasicOpenLibertyApplicationImageStreams(t, f, name, ns, 1)
	ol.Spec.ApplicationImage = imgstreamName

	timestamp := time.Now().UTC()
	t.Logf("%s - Creating ol application...", timestamp)
	err = f.Client.Create(goctx.TODO(), ol, &framework.CleanupOptions{TestContext: ctx, Timeout: time.Second, RetryInterval: time.Second})
	if err != nil {
		return err
	}
	...
	// Get the application
	f.Client.Get(goctx.TODO(), types.NamespacedName{Name: name, Namespace: ns}, ol)
	if err != nil {
		t.Fatal(err)
	}

	firstImage := ol.Status.ImageReference
	...
	secondImage := ol.Status.ImageReference
