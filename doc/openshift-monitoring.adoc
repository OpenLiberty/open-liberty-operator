...
Prior to installing application monitoring on RHOCP, ensure that there is a running application that has a service endpoint for outputting metrics in Prometheus format inside your project namespace. This guide assumes the service endpoint that outputs Prometheus format metrics is exposed on path `/metrics`.
