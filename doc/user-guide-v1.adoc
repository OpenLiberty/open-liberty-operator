...
- Network policies are created for each application to block incoming traffic. See link:++https://www.ibm.com/docs/en/was-liberty/base?topic=operator-configuration-examples#cfg-r-commoncomp__cc-networkpol++[Allowing or limiting incoming traffic (.spec.networkPolicy)]
