...
You can specify the size of the persisted storage to request using `serviceability.size` parameter. You can also specify which storageClassName to use if you don't want to use the default storageClass. The operator will automatically create a `PersistentVolumeClaim` with the specified size and access modes `ReadWriteMany` and `ReadWriteOnce`. It will be mounted at `/serviceability` inside all Pods of the `OpenLibertyApplication` instance.
