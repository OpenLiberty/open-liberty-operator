...
The operator can request a client Id and client Secret from providers, rather than requiring them in advance. This can simplify deployment, as the provider's administrator can supply the information needed for registration once, instead of clientIds and secrets repetitively.  An additional secret specified by `sso.oidc[].autoRegisterSecret` contains the information necessary to perform the registration.  Automatic registration takes precedence over the values in the sso secret. This is tested with Red Hat Single Sign-on (RHSSO). 
...
data:
  ...
  # others (IBM Cloud Identity) may require a dedicated clientId and clientSecret for registration
  clientId: bW9vb29vb28=
  clientSecret: dGhlbGF1Z2hpbmdjb3c=
  #
  # IBM Cloud Identity requires the registration URL to be specified.
  # registrationURL: base64 data here
...
Note: for RHSSO, set the oidc parameter userNameAttribute to preferred_username to obtain the user ID that was used to log in.
