...
data:
  ...
  #
  # IBM Cloud Identity requires a speicial clientId and clientSecret for registration, and the registration URL.
...
The following example uses automatic registration.
...
spec:
  ...
  sso:    
    oidc:     
      - discoveryEndpoint: https://sso-rhsso.apps.testcluster051320.os.fyre.ibm.com/auth/realms/sso_realm/.well-known/uma2-configuration        
        autoRegisterSecret: my-autoreg-secret
----
