...
func GetPathFromLeafIndex(treeMap map[string]interface{}, version string, index int) (string, error) {
	...
	return "", fmt.Errorf("could not find leaf index %s in treeMap", fmt.Sprint(index))
}
...
func GetLabelFromDecisionPath(operandVersionString string, pathOptions []string, pathChoices []string) (string, error) {
	...
	nc := len(pathChoices)
	if no == 0 || nc == 0 {
		return "", fmt.Errorf("expected decision tree path lists to be non-empty but got arrays of length %s and %s", fmt.Sprint(no), fmt.Sprint(nc))
	}
	if no != nc {
		return "", fmt.Errorf("expected decision tree path list to be the same length but got arrays of length %s and %s", fmt.Sprint(no), fmt.Sprint(nc))
	}
