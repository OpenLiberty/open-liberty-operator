...
[source,sh]
----
kubectl apply --server-side -f https://raw.githubusercontent.com/OpenLiberty/open-liberty-operator/main/internal/deploy/kubectl/openliberty-app-crd.yaml
----
...
+
  * To optionally reconcile multiple `OpenLibertyApplication` instances concurrently, set `MAX_CONCURRENT_RECONCILES=<SPECIFY_MAX_CONCURRENT_RECONCILES_HERE>` (default is `1`)
+
[source,sh]
----
...
[source,sh]
----
curl -L https://raw.githubusercontent.com/OpenLiberty/open-liberty-operator/main/internal/deploy/kubectl/openliberty-app-rbac-watch-all.yaml \
...
[source,sh]
----
curl -L https://raw.githubusercontent.com/OpenLiberty/open-liberty-operator/main/internal/deploy/kubectl/openliberty-app-rbac-watch-another.yaml \
      | sed -e "s/OPEN_LIBERTY_OPERATOR_NAMESPACE/${OPERATOR_NAMESPACE}/" \
...
[source,sh]
----
curl -L https://raw.githubusercontent.com/OpenLiberty/open-liberty-operator/main/internal/deploy/kubectl/openliberty-app-operator.yaml \
      | sed -e "s/OPEN_LIBERTY_WATCH_NAMESPACE/${WATCH_NAMESPACE}/" \
      | sed -e "s/OPEN_LIBERTY_MAX_CONCURRENT_RECONCILES/'${MAX_CONCURRENT_RECONCILES}'/" \
