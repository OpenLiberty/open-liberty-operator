...
func (r *ReconcileOpenLiberty) generateLTPAKeys(instance *olv1.OpenLibertyApplication, ltpaMetadata *lutils.LTPAMetadata, ltpaConfigMetadata *lutils.LTPAMetadata) (string, string, error) {
	...
	if err != nil && kerrors.IsNotFound(err) {
		...
		if !thisInstanceIsLeader {
			return "", "", fmt.Errorf("Waiting for OpenLibertyApplication instance '%s' to generate the shared LTPA keys file for the namespace '%s'.", leaderName, instance.Namespace)
		}
		...
		if err != nil {
			...
			if err == nil {
				...
				if !found {
					...
					return "", "", fmt.Errorf("the LTPA Keys Creation ConfigMap is missing key %s", lutils.LTPAKeysCreationScriptFileName)
				}
				...
				if string(script) != savedScript {
					...
					return "", "", fmt.Errorf("the LTPA Keys Creation ConfigMap key '%s' is out of sync", lutils.LTPAKeysCreationScriptFileName)
				}
			...
			if err != nil {
				...
				if err == nil {
					...
					if !found {
						...
						return ltpaXMLSecret.Name, fmt.Errorf("the LTPA Config Creation ConfigMap is missing key %s", lutils.LTPAConfigCreationScriptFileName)
					}
