...
To leverage the use of these dashboards the logging events must be emitted in JSON format into standard-out. For information regarding how to configure an Open Liberty image with JSON logging please see [this article](https://github.com/OpenLiberty/ci.docker#logging).
...
Configure your Open Liberty docker image to enable MicroProfile Metrics 1.x by setting the `ARG` value with `MP_MONITORING=true` before building the image with the provided build process [here](https://github.com/OpenLiberty/ci.docker).
...
The above guided process provided by the `ci.docker` repository does not configure your Open Liberty server for secured access. Please see the below section on `MicroProfile Metrics 2.0` for manual steps on building and configuring your own Open Liberty docker image with authentication. You will need to substitute the `mpMetrics-2.0` to `mpMetrics-1.1`.
...
The above `server.xml` configuration secures access to the server with basic authentication using the `<quickStartSecurity>` element. The `<quickStartSecurity>` is used in the above example for simplicity. When configuring your server you may wish to use a [basic registry](https://www.ibm.com/support/knowledgecenter/en/SSEQTP_liberty/com.ibm.websphere.wlp.doc/ae/twlp_sec_basic_registry.html) or a [LDAP registry](https://www.ibm.com/support/knowledgecenter/en/SSEQTP_liberty/com.ibm.websphere.wlp.doc/ae/twlp_sec_ldap.html) for securing authenticated access to your server. When using Prometheus to scrape data from the `/metrics` endpoint only the _Service Monitor_ approach can be configured to negotiate authentication with the Open Liberty server. If you are planning to use the _legacy_ approach which does not authenticate you can configure your `server.xml` without authentication:
...
See [this](ttps://github.com/OpenLiberty/open-liberty-operator/blob/master/deploy/crds/full_cr.yaml) for full template of available fields.


### Visualizing your data with Grafana


There are IBM provided Grafana dashboards that leverage metrics from the JVM as well as from the Open Liberty runtime.  Details regarding how to deploy and configure Grafana are covered in the following [document](https://kabanero.io/guides/app-monitoring.html#deploy-grafana).
...
Enable the MicroProfile Health in your Open Liberty Operator YAML configuration file, with the following snippet. This sets both the Kubernetes Liveness and Readiness probes to the /health endpoint.
