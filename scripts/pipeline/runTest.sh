...
if [[ "$rc" == 0 ]]; then
    ...
    echo "debug of cluster may be required, issue @ebc debug $olo_demand_id in #was-ebc channel to keep cluster for debug"
    echo "issue @ebc debugcomplete $olo_demand_id when done debugging in #was-ebc channel "
    ...
    curl -X POST -H 'Content-type: application/json' --data '{"text":"issue @ebc debug '$olo_demand_id' in #was-ebc channel to keep cluster for debug"}' $(get_env slack_web_hook_url) </dev/null
