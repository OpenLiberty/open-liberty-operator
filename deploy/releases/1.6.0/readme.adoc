...
To install using the OpenShift UI, select `Ecosystem` from the navigation panel, then select `Software Catalog` and then search for `Open Liberty`. Click on the tile with `Open Liberty` to install the Operator. Select channel `v1.6` and install it.
