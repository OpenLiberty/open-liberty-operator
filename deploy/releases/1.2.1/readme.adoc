...
To install using the integrated OperatorHub in OpenShift UI, select `Operators` from the navigation panel, then select `OperatorHub` and then search for `Open Liberty`. Click on the tile with `Open Liberty` to install the Operator. Select the channel `v1.2` to install version 1.2.1.
